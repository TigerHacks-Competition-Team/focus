...
export const calculateGaze = (face) => {
  ...
  const eyeCenter = left // eye center is average of extreme points on x axis for both x and y, ignoring y extreme points as eyelids naturally open/close more when gazing up/down so relative point is less precise
    ? [
        (face.mesh[133][0] + face.mesh[33][0]) / 2,
        (face.mesh[133][1] + face.mesh[33][1]) / 2,
      ]
    : [
        (face.mesh[263][0] + face.mesh[362][0]) / 2,
        (face.mesh[263][1] + face.mesh[362][1]) / 2,
      ];
  const eyeSize = left // eye size is difference between extreme points for both x and y, used to normalize & squarify eye dimensions
    ? [
        face.mesh[133][0] - face.mesh[33][0],
        face.mesh[23][1] - face.mesh[27][1],
      ]
    : [
        face.mesh[263][0] - face.mesh[362][0],
        face.mesh[253][1] - face.mesh[257][1],
      ];
  ...
  strength = Math.min(
    strength,
    face.boundingBox.bottomRight[1] / 2,
    face.boundingBox.topLeft[0] / 2
  ); // limit strength to half of box size to avoid clipping due to low precision
...
export const calculateFaceAngle = (face, imageSize) => {
  ...
  const size =
    Math.max(
      face.boundingBox.bottomRight[1] * imageSize[0],
      face.boundingBox.topLeft[0] * imageSize[1]
    ) / 1.5;
  ...
  const gaze =
    mesh.length === 478 ? calculateGaze(face) : { bearing: 0, strength: 0 };
